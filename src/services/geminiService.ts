...
// Use a more stable model name or the recommended one for basic tasks
...
export const chatWithAitana = async (message: string, history: { role: 'user' | 'model', parts: { text: string }[] }[]) => {
  ...
  if (!apiKey) {
    throw new Error("La clave API de Gemini no está configurada. Por favor, revísala en la configuración de secretos.");
  }
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes("API_KEY_INVALID")) {
      throw new Error("La clave API de Gemini no es válida.");
    }
    throw error;
  }
};
